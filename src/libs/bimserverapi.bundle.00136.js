...
var BimServerApiWebSocket = function () {
	...
	createClass(BimServerApiWebSocket, [{
		...
		value: function connect() {
			...
			var location = this.bimServerApi.baseUrl.toString().replace('http://', 'ws://').replace('https://', 'wss://') + "/stream";
				...
				_this5.bimServerApi.callWithFullIndication("ServiceInterface", "download", {
					...
					_this5.bimServerApi.registerProgressHandler(topicId, function (topicId, state) {
						if (state.title == "Done preparing" && !handled) {
							...
							_this5.bimServerApi.notifier.setInfo("Getting model data...", -1);
							_this5.bimServerApi.getJson(url, null, function (data) {
								...
								_this5.bimServerApi.call("ServiceInterface", "cleanupLongAction", {
									...
									promise.fire();
									_this5.bimServerApi.notifier.setSuccess("Model data successfully downloaded...");
								});
							});
	...
	createClass(BimServerClient, [{
		...
		value: function login(username, password, callback, errorCallback, options) {
			...
			this.call("AuthInterface", "login", request, function (data) {
				...
				if (options.done !== false) {
					_this3.notifier.setInfo("Login successful", 2000);
				}
				_this3.resolveUser();
				_this3.webSocket.connect(callback);
		...
		value: function logout(callback) {
			...
			this.call("AuthInterface", "logout", {}, function () {
				_this5.notifier.setInfo("Logout successful");
				callback();
			});
		}
	}, {
		...
		key: 'multiCall',
		value: function multiCall(requests, callback, errorCallback, showBusy, showDone, showError) {
			var _this17 = this;
			...
			this.getJson(this.address, request, function (data) {
				...
				if (textStatus == "abort") {
					...
					_this17.notifier.setError("ERROR_REMOTE_METHOD_CALL");
				}
				if (callback != null) {
					var result = {};
...
//# sourceMappingURL=bimserverapitranspiled.js.map
